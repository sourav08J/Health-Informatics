// resp.js - Respiratory Analysis + Gemini Medicine Integration
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function provideAdvice() {
        ...
        /* ---------- UI: Loading ---------- */
        // Teal color theme for Respiratory
        responseBox.innerHTML = "🫁 Analyzing respiratory symptoms...";
        responseBox.style.color = "#16a085";
        responseBox.style.opacity = 0.6;

        medicineBox.innerHTML = "💊 Generating medicine suggestions...";
        medicineBox.style.color = "#16a085";
        medicineBox.style.opacity = 0.6;

        /* ===============================
           4. CSV-BASED RESPIRATORY ADVICE
        =============================== */
        fetch("http://127.0.0.1:5000/analyze_respiratory", {
            method: "POST",
            headers: {
        ...
        .then(data => {
            ...
            if (!data.answer || data.answer.trim() === "") {
                responseBox.innerHTML =
                    "ℹ️ No matching respiratory condition found. Please consult a doctor.";
        ...
        .catch(err => {
            console.error("Respiratory CSV Error:", err);
            responseBox.innerHTML = "❌ Respiratory database not reachable.";
        ...
        =============================== */
        // Note: You must add the '/respiratory_medicine' route to app.py!
        fetch("http://127.0.0.1:5000/respiratory_medicine", {
            method: "POST",
            headers: {
                "Content-Type": "application/json"
            },
            body: JSON.stringify({
                disease: "Respiratory/Lung Issue", // Context for AI
                symptoms: query
            })
        })
        .then(res => {
