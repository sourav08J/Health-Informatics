// blood.js - CSV Blood Analysis + Gemini Medicine Integration
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function provideAdvice() {
        ...
        /* ---------- UI: Loading ---------- */
        // Using RED color for Blood theme
        responseBox.innerHTML = "🩸 Analyzing blood condition...";
        ...
        /* ===============================
           4. CSV-BASED BLOOD ADVICE
        =============================== */
        fetch("http://127.0.0.1:5000/analyze_blood", {
            method: "POST",
            headers: {
        ...
        .then(data => {
            ...
            if (!data.answer || data.answer.trim() === "") {
                responseBox.innerHTML =
                    "ℹ️ No matching blood condition found. Please consult a doctor.";
                responseBox.style.color = "#7f8c8d";
            } else {
                responseBox.innerHTML = data.answer;
                responseBox.style.color = "#c0392b"; // Red text for diagnosis
            }
        })
        .catch(err => {
            console.error("CSV Error:", err);
            responseBox.innerHTML = "❌ Blood database not reachable.";
            responseBox.style.color = "#c0392b";
            responseBox.style.opacity = 1;
        });
        ...
        =============================== */
        // Note: You must add the '/blood_medicine' route to app.py for this to work!
        fetch("http://127.0.0.1:5000/blood_medicine", {
            method: "POST",
            headers: {
                "Content-Type": "application/json"
            },
            body: JSON.stringify({
                disease: "Blood Disorder", // Specific context for AI
                symptoms: query
            })
        })
        .then(res => {
        ...
        .then(data => {
            ...
            if (!data.medicine || data.medicine.trim() === "") {
                ...
                medicineBox.style.color = "#2c3e50"; // Keep medicine text dark for readability
            }
        })
        .catch(err => {
